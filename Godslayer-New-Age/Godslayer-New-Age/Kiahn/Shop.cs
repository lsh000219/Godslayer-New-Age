...
namespace Godslayer_New_Age
{
    internal class Shop
    {
        ...
        // {
        //     "`red,blue`a1", "a2", "`null,green`a3" }; << maximum 25lines
        // PrintDB.itemDataText = aaa;

        //element) "`red,blue`a1" << print "a1", font color: red, background color: blue
        // ` << backtick

        List<string> itemDataText = new List<string>();
        ...
        public void Display()
        {
            ...
            {
                itemDataText.Add($"`gray,black`[{item.TypeName}] `white,black`{item.Name} `yellow,black`{item.PassiveDesc}");

                if (item.TypeName == "무기")
                {
                    itemDataText.Add($"`red,black`ATK: {item.Atk}");
                }
                else if (item.TypeName == "방어구")
                {
                    itemDataText.Add($"`cyan,black`DEF: {item.Def}");
                }
                else if (item.TypeName == "장신구")
                {
                    if (item.Atk == 0 && item.Def == 0) { itemDataText.Add($""); }
                    else
                    {
                        itemDataText.Add($"`red,black`ATK: {item.Atk} `cyan,black`DEF: {item.Def}");
                    }
                }
                itemDataText.Add($"`white,black`[{item.Desc}] `yellow,black`{item.Price} G");
